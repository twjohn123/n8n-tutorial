...
首先一樣首先打開 Windows 的**命令提示字元** (可以在搜尋欄中輸入cmd來開啟)，接著輸入以下指令：
    ...
    docker run --name PostgreSQL-school -e POSTGRES_USER=myuser -e POSTGRES_PASSWORD=mypassword -e POSTGRES_DB=schooldb -p 5432:5432 -v postgres_data:/var/lib/postgresql -d postgres
...
接著輸入原本創建 n8n 和 PostgreSQL 容器的指令，但在裡面再加上 "--network my_internal_net"
    ...
    docker run --name PostgreSQL-school --network my_internal_net -e POSTGRES_USER=myuser -e POSTGRES_PASSWORD=mypassword -e POSTGRES_DB=schooldb -p 5432:5432 -v postgres_data:/var/lib/postgresql -d postgres
...
* 中間的 Merge 節點用來整合各個分支的資料，以便在 Gmail 節點直接挪用。

*  將信件分批寄送給被篩選出的助理：此部分使用迴圈分批對各行資料進行處理，並將郵件發送給特定助理。
  * Loop Over Items：此處設定一次僅允許一個物件 (一行資料) 通過，若網路環境良好，可將此節點的 "**Batch Size**" 調高，一次允許更多物件通過，以加快執行速度。
  * 寄給助理：此節點將把郵件發給特定對象，信件標題和內文皆會受到前面節點的影響 (月份、收件人...)。
  * Wait：在執行下個迴圈前等待一段時間，避免 API 短時間內被呼叫太多次，進而拒絕節點的呼叫。

*  將信件分批寄送給被篩選出的教授：此部分和寄給助理的流程基本一致，差別只在寄送 Gmail 節點的內文，一個是內文內的稱呼為「助理」，一個稱呼為「教授」。

如此，兩個工作流便都探討完了。



<h2 id="result">校務自動化的應用實例</h2>
